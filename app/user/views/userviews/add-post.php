<div id="initial-card" class="bg-gray-100 dark:bg-slate-700 shadow-lg ml-20 mr-10 w-9/12 min-h-56 flex justify-start items-start relative rounded-xl">
    ...
    <form class="requestForm w-full" action="<?php echo APP_URL; ?>user/requestControllers/post/postRequest.php" method="POST" autocomplete="off" enctype="multipart/form-data">
        ...
        <textarea id="post-content" class="relative rounded-lg px-6 py-2 top-7 mb-10 left-20 text-2xl text-gray-400 bg-gray-100 dark:bg-slate-700 resize-none outline-none font-semibold w-11/12 h-auto" name="content">What's in your mind?</textarea>
        <div>
        ...
        <div class="flex absolute bottom-3 left-8 w-8/12">
            ...
            <select id="countries" class="bg-gray-200 border border-gray-300 text-gray-500 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-3/12 p-2.5 dark:bg-slate-600 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 cursor-pointer ml-2 font-semibold">
                <option selected>Choose a category</option>
                <option value="US">Zero Hunger</option>
                <option value="CA">Quality Education</option>
                <option value="FR">Gender Equality</option>
                <option value="DE">Clean Energy</option>
            </select>
        </div>
        ...
        <div class="flex absolute bottom-3 right-5 gap-2">
            ...
            <button type="submit" id="share" class="text-white font-semibold bg-blue-400 dark:bg-blue-500 rounded-full px-7 py-1 text-center">Share</button>
        </div>
    </form>
</div>
<script>
    ...
    textArea.addEventListener('input', () => {
        const value = textArea.value.length;
         ...
         textArea.style.height = 'auto'; // Revertimos la altura a auto
        textArea.style.height = textArea.scrollHeight + 'px'; // Ajustamos la altura según el contenido del textarea
    });
