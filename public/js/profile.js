const profile = {
  ...
  userPosts: function () {
    ...
    `;
    this.up.html("");
    fetch(this.routes.userPosts)
      .then((res) => res.json())
      .then((posts) => {
        usp = posts;
        html = "";
        posts.forEach((post) => {
          html += this.userPostsHtml(post);
        });
        this.up.html(html);
      });
  ...
  userSharedPosts: function () {
    ...
    `;
    this.up.html("");
    fetch(this.routes.userSharedPosts)
      .then((res) => res.json())
      .then((posts) => {
        usp = posts;
        const allSharedPosts = posts.filter((post) => post.postId !== null);
        if (allSharedPosts.length > 0) {
          html = "";
          allSharedPosts.forEach((post) => {
            html += this.sharedPostHtml(post);
          });
          this.up.html(html);
        }
      });
  ...
  userProfile: function () {
    ...
    fetch(this.routes.userProfile)
      ...
      .then((u) => {
        const user = u[0];
        let html = this.profileHtmlBuilder(user);
        ...
        function loadColorFromLocalStorage() {
          ...
          if (savedColor) {
            const currentClass = Array.from(backgroundClasses).find((cls) =>
              cls.startsWith("from-")
            );
            if (currentClass) {
              backgroundClasses.replace(currentClass, savedColor);
            } else {
              backgroundClasses.add(savedColor);
            }
          }
        }
        ...
        function changeBackgroundColor(colorClass) {
          ...
          if (currentClass) {
            backgroundClasses.replace(currentClass, colorClass);
          } else {
        ...
        yellow.on("click", () => changeBackgroundColor("from-yellow-300"));
        purple.on("click", () => changeBackgroundColor("from-purple-500"));
        fuchsia.on("click", () => changeBackgroundColor("from-fuchsia-400"));
        orange.on("click", () => changeBackgroundColor("from-orange-400"));
        gray.on("click", () => changeBackgroundColor("from-gray-300"));
  ...
  profileHtmlBuilder: function (user) {
    return `
 ...
 <section id="modal" class="profile-modal bg-gray-100 dark:bg-slate-700 rounded-xl w-11/12 lg:w-8/12 mx-auto relative mt-10 overflow-y-auto overflow-x-hidden lg:overflow-hidden">
   ...
   <section class="flex">
     <form class="flex w-full lg:flex-row flex-col" id="updateUser-form" method="POST" autocomplete="off" enctype="multipart/form-data">
       <div class="ml-16">
         <h1 class="text-black text-2xl font-bold mt-5 ml-5 mb-5 dark:text-white text-start mr-2">Upload your <span class="text-blue-500"> information</span></h1>
         ...
         <section class="flex gap-5">
           ...
           <input name="nacionality" id="nacionality" type="text" placeholder="Nacionality" class="border-b-2 border-t-1 border-gray-300 py-3 focus:border-b-2 focus:border-gray-400 transition-colors focus:outline-none bg-inherit bg-slate-200 dark:bg-slate-600 rounded-2 xl px-5 text-gray-400 font-semibold w-5/12 mt-5 shadow-md rounded-xl" ${
             user.nacionality !== null ? 'value="' + user.nacionality + '"' : ""
           } />
         </section>
       ...
       <section class="w-11/12 mx-auto lg:w-6/12 flex-col sm:flex-row flex lg:flex-col items-center">
         <div>
           <h1 class="text-black text-2xl font-bold mt-5 ml-5 mb-5 dark:text-white text-center mr-2">Upload your profile <span class="text-blue-500"> image</span></h1>
         ...
         <div class="w-7/12">
           <h1 class="text-black text-2xl font-bold mt-5 ml-5 mb-5 dark:text-white text-center mr-2">Choose your <span class="text-blue-500"> color</span></h1>
           <div class="w-12/12 relative h-28 2xl:h-32 shadow-xl mt-5 rounded-xl flex mb-10">
             <div id="red" class="relative bg-red-500 w-4/12 h-full rounded-l-lg hover:scale-110"></div>
             <div id="green" class="relative bg-green-400 w-4/12 h-full hover:scale-110"></div>
             <div id="blue" class="relative bg-sky-500 w-4/12 h-full hover:scale-110"></div>
             <div id="yellow" class="relative bg-yellow-300 w-4/12 h-full hover:scale-110"></div>
             <div id="purple" class="relative bg-purple-500 w-4/12 h-full hover:scale-110"></div>
             <div id="fuchsia" class="relative bg-fuchsia-400 w-4/12 h-full hover:scale-110"></div>
             <div id="orange" class="relative bg-orange-400 w-4/12 h-full hover:scale-110"></div>
             <div id="gray" class="relative bg-gray-300 w-4/12 h-full hover:scale-110"></div>
             <div id="black" class="relative bg-black w-4/12 h-full hover:scale-110"></div>
             <div id="white" class="relative bg-white w-4/12 h-full rounded-r-lg hover:scale-110"></div>
           </div>
         </div>
       </section>
     </form>
 ...
 `;
  ...
  <span id="comments" class="comments_count${post.id}" data-countc="${post.comments}">${post.comments}</span>`
      : "";
    let hashtagsHTML = app.buildHashtagsHtml(post.hashtag);
    return `
...
<div class="flex gap-4 w-10/12 mx-auto mt-5">
${hashtagsHTML}
</div>    <input type="hidden" name"category" value="${post.category}">
<div class="flex w-10/12 relative mt-10 mb-5 mx-auto">

    <div data-post-id="${
      post.id
    }" class="like text-gray-400 w-8 h-8 transition-all cursor-pointer">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
            <path class="fill-current" d="M47.6 300.4L228.3 469.1c7.5 7 17.4 10.9 27.7 10.9s20.2-3.9 27.7-10.9L464.4 300.4c30.4-28.3 47.6-68 47.6-109.5v-5.8c0-69.9-50.5-129.5-119.4-141C347 36.5 300.6 51.4 268 84L256 96 244 84c-32.6-32.6-79-47.5-124.6-39.9C50.5 55.6 0 115.2 0 185.1v5.8c0 41.5 17.2 81.2 47.6 109.5z" />
        </svg>
${likesHtml}
  ...
  sharedPostHtml: function (post) {
    ...
    let orighashtagsHTML = app.buildHashtagsHtml(post.originalhtsg);
    return `
    <div data-postsection class="post">
...
$("#userSharedPosts").on("click", function () {
  $(function () {
    profile.userSharedPosts();
  });
});