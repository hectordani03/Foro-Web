          ...
          .then((res) => {
            if (res.r !== false) {
              Swal.fire({
                ...
                updateUserInfoInDOM(res.data); // Llamada a la nueva función
              });
            } else {
...
// Nueva función para actualizar la información del usuario en el DOM
function updateUserInfoInDOM(userData) {
  // Actualiza los campos del formulario con los nuevos datos
  $("#usernamep").val(userData.username);
  $("#emailp").val(userData.email);
  $("#age").val(userData.age);
  $("#nacionality").val(userData.nacionality);
  $("#description").val(userData.description);

  // Actualiza la imagen de perfil si se ha cargado una nueva
  if (userData.profilePicture) {
    // Actualiza la imagen en el DOM
    $("#img-view").html(`<img src="${userData.profilePicture}" alt="Profile Picture" />`);
  }

  // Aquí puedes agregar más actualizaciones según tus necesidades
}