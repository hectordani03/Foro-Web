document.addEventListener("DOMContentLoaded", function () {
  ...
  const bcontent = document.getElementById("body-content");

  commentButtons.forEach((commentBtn) => {
    commentBtn.addEventListener("click", (e) => {
      const Id = commentBtn.getAttribute("data-post-id");
      const postContainer = commentBtn.closest(".post");

      const content = postContainer
        .querySelector("[data-post-cont]")
        .getAttribute("data-post-cont");
      const category = postContainer
        .querySelector("[data-post-category]")
        .getAttribute("data-post-category");
      const profile_picture = postContainer
        .querySelector("[data-post-profpic]")
        .getAttribute("data-post-profpic");
      const username = postContainer
        .querySelector("[data-post-username]")
        .getAttribute("data-post-username");
      const time = postContainer
        .querySelector("[data-post-time]")
        .getAttribute("data-post-time");
        let imgElement = postContainer.querySelector("[data-post-img]");
        let img = imgElement ? imgElement.getAttribute("data-post-img") : null;
        

        if (img !== null) {
          img = `../assets/post_img/${img}`;
        }
      ...
      $section.innerHTML = `
            ...
            <div class="absolute right-5 top-0">
                    ...
                    <img class="w-12 h-10 bg-blue-500 rounded-full top-8 left-8" src="../assets/profile_picture/${profile_picture}" alt="">
                    <div class="rounded-xl bg-gray-200 dark:bg-slate-600 shadow-lg">
                    <div class="ml-5 mr-5 mt-1 mb-2">
                    <p class="text-gray-400 font-bold">${username}</p>
                    ...
                    <div class="flex gap-3 absolute right-3">
                        ...
                        <img class="w-12 h-11 bg-blue-500 rounded-full top-8 left-8" src="../assets/profile_picture/${currentUser.profile_picture}" alt="">
                        ...
                        <script>
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path class="fill-current" d="M498.1 5.6c10.1 7 15.4 19.1 13.5 31.2l-64 416c-1.5 9.7-7.4 18.2-16 23s-18.9 5.4-28 1.6L284 427.7l-68.5 74.1c-8.9 9.7-22.9 12.9-35.2 8.1S160 493.2 160 480V396.4c0-4 1.5-7.8 4.2-10.7L331.8 202.8c5.8-6.3 5.6-16-.4-22s-15.7-6.4-22-.7L106 360.8 17.7 316.6C7.1 311.3.3 300.7 0 288.9s5.9-22.8 16.1-28.7l448-256c10.7-6.1 23.9-5.5 34 1.4z"/></svg>
                            </button>
                            </div>
                            </section>
                            </section>`;

      const closeButtons = $section.querySelectorAll("#close");
      closeButtons.forEach((closeBtn) => {
        closeBtn.addEventListener("click", () => {
          ...
          if (modal && capa) {
            ...
            bcontent.classList.remove("overflow-hidden");
            bcontent.classList.add("overflow-auto");
          }
        });
      });
      document.body.appendChild($section);
      bcontent.classList.add("overflow-hidden");
    });
  });
});
